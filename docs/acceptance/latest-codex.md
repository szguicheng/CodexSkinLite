...
Status: Developer ID signed/icon build ready; not notarized; full user retest pending

## Candidate

- Source commits: `e51847bd`, `5a487341`, `75b1a956`, `b34da733`, `fa747f78`, `f95c4956`, `8f4acade`, `33ef9322`, `130c33cc`, `8afb9e6b`, `e043ec09`, `1f925662`
- macOS: 26.5.2 (25F84)
- Official Codex desktop bundle: `/Applications/ChatGPT.app`
- Bundle ID: `com.openai.codex`
- Codex version/build: 26.825.51511 (7377)
- ZIP: `dist/CodexSkinLite-0.1.9-macos-arm64.zip`
- SHA-256: `ae43d096f9592b6eb5aa5b1018cecc0f18d79dd1bb1aedbc6317b1a1e819924f`
- Bundle: `0.1.9 (10)`, Mach-O arm64
- Renderer API/bytes: `11` / `23,757`
- Renderer timers: 0
...
- Keep the theme background visible when Codex loses focus by painting the managed image directly on `html, body`; use a background-only gradient for image opacity and avoid a negative z-index compositing layer. Scroll the native customization editor to its background-image controls on open.
- Preserve the imported theme's component shadows by leaving its CSS unchanged; the previous negative background layer was the compositing regression that made those shadows look abnormal.
- Generate a multi-size `CodexSkinLite.icns`, declare it in the bundle plist, and sign the finished App with the first available Developer ID Application identity; do not upload it for notarization in this candidate.
...
3. Extract `CodexSkinLite-0.1.9-macos-arm64.zip` and replace the old `CodexSkinLite.app` before opening it.
4. If Gatekeeper shows an unidentified-developer warning, use Finder’s Open action once; this candidate is signed but not notarized.
...
| E1 | Launch CodexSkinLite from the new 0.1.9 bundle | Settings opens once automatically and is in front; closing it does not create another window | Pass in prior startup smoke; user retest pending |
| E8 | Inspect the new 0.1.9 bundle in Finder | The CodexSkinLite icon is shown; the App is Developer ID signed and has not been submitted for notarization | Local `codesign` and `spctl` checks pass; user retest pending |
